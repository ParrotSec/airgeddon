...
function set_language_strings_version() {
	...
	language_strings_version="9.21-1"
}
...
function language_strings() {
	...
	arr["RUSSIAN",613]="Предупреждение. Пожалуйста, проверьте файл ${rc_file} или флаги командной строки. Недопустимое значение переменной ${normal_color}${error_var_name}${yellow_color}. Будет использовано значение по умолчанию ${normal_color}${error_var_default_value}${yellow_color}"
	arr["GREEK",613]="Προειδοποίηση. Ελέγξτε το αρχείο ${rc_file} ή τα flags της γραμμής εντολών. Βρέθηκε μη έγκυρη τιμή για τη μεταβλητή ${normal_color}${error_var_name}${yellow_color}. Θα χρησιμοποιηθεί η προεπιλεγμένη τιμή ${normal_color}${error_var_default_value}${yellow_color}"
	...
	arr["RUSSIAN",614]="Предупреждение. Пожалуйста, проверьте файл ${rc_file}. Конфигурационная переменная ${normal_color}${error_var_name}${yellow_color} не найдена. Будет использоваться значение по умолчанию ${normal_color}${error_var_default_value}${yellow_color}"
	arr["GREEK",614]="Προειδοποίηση. Ελέγξτε το αρχείο ${rc_file}. Η μεταβλητή για το configuration ${normal_color}${error_var_name}${yellow_color} δεν βρέθηκε. Θα χρησιμοποιηθεί η προεπιλεγμένη τιμή  ${normal_color}${error_var_default_value}${yellow_color}"
	...
	arr["GREEK",615]="Υπάρχει ένα σφάλμα στο configuration σας. Ελέγξτε το αρχείο ${rc_file}. Έχετε ενεργοποιήσει την επιλογή που εξαναγκάζει τη χρήση του iptables αντί του nftables (${normal_color}AIRGEDDON_FORCE_IPTABLES${red_color}) και το σύστημά σας δεν έχει το έχει εγκατεστημένο (iptables). Απενεργοποιήστε την επιλογή αυτή ή εγκαταστήστε το iptables για να επιλυθεί το πρόβλημα"
	...
	arr["CATALAN",618]="Maneig de finestres: ${pink_color}xterm${normal_color}"
	arr["PORTUGUESE",618]="Gerenciador de janelas: ${pink_color}xterm${normal_color}"
	arr["RUSSIAN",618]="Управление окнами: ${pink_color}xterm${normal_color}"
	...
	arr["CATALAN",619]="Maneig de finestres: ${pink_color}tmux${normal_color}"
	arr["PORTUGUESE",619]="Gerenciador de janelas: ${pink_color}tmux${normal_color}"
	arr["RUSSIAN",619]="Управление окнами: ${pink_color}tmux${normal_color}"
	...
	arr["FRENCH",620]="La méthode de gestion des fenêtres a été modifiée dans le fichier de configuration. Redémarrez le script pour que les modifications prennent effet"
	arr["CATALAN",620]="S'ha modificat en el fitxer de configuració el mètode de gestió de finestres. Reinicia el script perquè els canvis tinguin efecte"
	arr["PORTUGUESE",620]="O gerenciador de janelas foi modificado no arquivo de configuração. Reinicie o script para que as alterações entrem em vigor"
	arr["RUSSIAN",620]="Метод управления окнами был изменён в файле конфигурации. Перезапустите скрипт, чтобы изменения вступили в силу"
	...
	arr["SPANISH",636]="Hay un error en tu configuración. Por favor revisa el fichero ${rc_file}. Has activado la opción para forzar el uso de mdk3 (${normal_color}AIRGEDDON_MDK_VERSION${red_color}) pero tu sistema no tiene mdk3 instalado. Deshabilita esta opción o instala mdk3 para solucionar el problema"
	...
	arr["CATALAN",636]="Hi ha un error en la configuració. Si us plau comprova el fitxer ${rc_file}. Has activat l'opció per forçar l'ús d'mdk3 (${normal_color}AIRGEDDON_MDK_VERSION${red_color}) però el teu sistema no té mdk3 instal·lat. Deshabilita aquesta opció o instal·la mdk3 per solucionar el problema"
	arr["PORTUGUESE",636]="Existe um erro na sua configuração. Por favor, verifique o arquivo ${rc_file}. Você ativou a opção para forçar o uso de mdk3 (${normal_color}AIRGEDDON_MDK_VERSION${red_color}), mas seu sistema não possui o mdk3 instalado. Desabilite esta opção ou instale o mdk3 para resolver o problema"
	arr["RUSSIAN",636]="В вашей конфигурации имеется ошибка. Пожалуйста, проверьте файл ${rc_file}. Вы включили опцию принудительного использования mdk3 (${normal_color}AIRGEDDON_MDK_VERSION${red_color}), но в вашей системе не установлена mdk3. Отключите эту опцию или установите mdk3, чтобы исправить проблему"
	...
	arr["FRENCH",652]="Les certificats ne sont pas valides ou le mot de passe est incorrect, donc l'attaque ne peut pas être effectuée"
	arr["CATALAN",652]="Els certificats no són vàlids o la contrasenya és errònia, per la qual cosa l'atac no es pot realitzar"
	arr["PORTUGUESE",652]="Os certificados são inválidos ou a senha está incorreta, o ataque não pode ser realizado"
	arr["RUSSIAN",652]="Сертификаты недействительны или неверный пароль, поэтому атака не может быть выполнена"
	...
	arr["FRENCH",653]="Entrez le chemin du répertoire dans lequel se trouvent les fichiers du certificats. ${blue_color}Assurez-vous que les fichiers ca.pem, server.pem et server.key existent dans le chemin indiqué:"
	arr["CATALAN",653]="Introdueix la ruta del directori on es troben els fitxers dels certificats. ${blue_color}Assegura't que els fitxers ca.pem, server.pem i server.key existeixen a la ruta indicada:"
	arr["PORTUGUESE",653]="Digite o caminho do diretório onde os arquivos de certificado estão localizados. ${blue_color}Certifique-se de que os arquivos ca.pem, server.pem e server.key existam no caminho indicado:"
